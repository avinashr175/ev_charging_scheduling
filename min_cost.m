function [y,soc_ev,obj,baseline_price] = min_cost(EV_count,T,cs_del_capacity,reg_price,elec_price,efficiency,start_time,end_time,soc_initial,soc_final,upper_charging_limit,lower_charging_limit,M,battery_capacities)
h = zeros(EV_count,T);
for i=1:EV_count
    for t=1:T
...
cvx_begin
    ...
    for t=1:T
        x = 0;
        for i=1:EV_count
            obj = obj + (M+elec_price(t))*charging_rate(i,t)*h(i,t);
...
for t=1:T
    for i=1:EV_count
        baseline_price = baseline_price + (M+elec_price(t))*charging_rates2(i,t)*h(i,t); 
    end
end
